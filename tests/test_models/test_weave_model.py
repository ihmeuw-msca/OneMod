...
def test_weave_model(sample_input_data, temporary_directory):
    ...
    regmod_smooth_path = temporary_directory / "results" / "regmod_smooth" / "predictions.parquet"
    ...
    sample_input_data['residual_se'] = (
            1 / np.sqrt(sample_input_data['pred_rate'] * (1 - sample_input_data['pred_rate'])))
    ...
    weave_model(experiment_dir=temporary_directory,
                submodel_id=submodel_id)
    expected_data_path = Path(
        temporary_directory / "results" / "weave" / "submodels" / f"{submodel_id}.parquet"
    )
