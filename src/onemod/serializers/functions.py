...
def serialize(obj: BaseModel | dict | List[BaseModel] | List[dict], filepath: Path | str) -> None:
    ...
    if isinstance(obj, BaseModel):
        ...
        if all(isinstance(item, BaseModel) for item in obj):
            ...
            raise TypeError("All items in the list must be either BaseModel instances or dicts.")
    else:
        raise TypeError("Object to serialize must be a BaseModel, dict, or a list of these.")
...
def _get_file_format(filepath: Path | str) -> str:
    ...
    if file_extension in {'.json'}:
        ...
        raise ValueError(f"Unsupported file format: {file_extension}. Supported formats are: .json, .yaml, .yml")
